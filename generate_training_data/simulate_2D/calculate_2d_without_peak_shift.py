...
def sum_mixture_for_each_repli(n, mixture_list, format_norm_data_dict, cons_table_rows, protons_df, snr, group_flag):
...
def simulate_mixture_for_all_repli(num_replicates, mixture_list, format_norm_data_dict, cons_table_rows, protons_df, snr, group_flag):
    replicate_dict = dict()
    for n in range(num_replicates):
        temp_sum_data = sum_mixture_for_each_repli(n, mixture_list, format_norm_data_dict, cons_table_rows, protons_df, snr, group_flag)
...
def conti_sum_mixture_for_each_repli(n, mixture_list, format_norm_data_dict, cons_table_rows, protons_df, snr):
    sum_data = 0
    for meta_name in mixture_list:
        temp_dict = list(filter(lambda t: t['meta_name'] == meta_name, cons_table_rows))[0]
...
def simulate_continuous_mixture_for_all_repli(num_replicates, mixture_list, format_norm_data_dict,
                                              cons_table_rows, protons_df, snr):
    replicate_dict = dict()
    for n in range(num_replicates):
        temp_sum_data = conti_sum_mixture_for_each_repli(n, mixture_list, format_norm_data_dict,
                                                         cons_table_rows, protons_df, snr)
        replicate_dict['replicate_' + str(n + 1)] = temp_sum_data
