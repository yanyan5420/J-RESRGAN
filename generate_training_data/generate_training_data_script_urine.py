...
def scale_HR_to_0_256(im):
    if np.max(im) < 256:
        scale_size = 256 // np.max(im)
        scale_im = im * scale_size
    else:
        scale_size = np.max(im) / 256
        scale_im = im / scale_size
    return scale_im
...
def generate_mixture_spectra(meta_name_list, meta_cons_df, whole_meta_spectra_dict, sample_size, filename):
    ...
    for i in range(whole_cons_df.shape[1]):
        ...
        image_lr = add_gaussian_filter(scaled_im, sigma=(5, 7))
        image_lr_ds = area_down_sampling(image_lr, 2)

        cv2.imwrite("/data/DL_JRes_NMR/Real-ESRGAN/datasets/DF2K/JRES_train_HR_r17/" + filename + '_' +
                    '{0}'.format(str(i+1).zfill(4)) + ".tif", scaled_im)
        cv2.imwrite("/data/DL_JRes_NMR/Real-ESRGAN/datasets/DF2K/JRES_train_LR_r17/" + filename + '_' +
